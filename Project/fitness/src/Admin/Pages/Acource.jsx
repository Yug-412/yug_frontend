...
function Acource() {
    ...
    const viewdata = async (id) => {
        const res = await axios.get(`http://localhost:3000/cource/${id}`)
        // console.log(res.data)
        setview(res.data)
    }
    ...
    const updatedata = async (e) => {
        e.preventDefault()
        try {
            const res = await axios.put(`http://localhost:3000/cource/${update.id}`, update)
            setupdate(res.data)
            fetchdata()
            const modalEl = document.getElementById("exampleModal1");
            const modelInstance = window.bootstrap.Modal.getInstance(modalEl);
            if (modelInstance) {
                modelInstance.hide()
            }
        } catch (error) {
    ...
    const deletedata = async(id)=>{
     await axios.delete(`http://localhost:3000/cource/${id}`)
                                ...
                                return (
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <button type="button" onClick={() => viewdata(user.id)} className="btn btn-info btn-md btn-rounded" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                                View
                                            </button>
                                            <button type="button" onClick={() => showData(user)} className="btn btn-success btn-md btn-rounded mx-2" data-bs-toggle="modal" data-bs-target="#exampleModal1">
                                                Edit
                                            </button>
                                            <button type="button" onClick={()=>deletedata(user.id)} className="btn btn-danger btn-md btn-rounded">
                                                Delete
                                            </button>
                                        </td>
                                    </tr>
